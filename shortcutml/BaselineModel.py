...
class BaselineModel:
    ...
    def evaluate(self, X_train, X_test, y_train, y_test):
        ...
        for model in self.get_models():
            ...
            if self.type == "regression":
                self.test_result.append([model[0], self.score(y_test, y_pred)])
        ...
        if self.type == "regression":
            self.test_result = pd.DataFrame(
                self.test_result, columns=["model", "rmse_score"]).sort_values(by="rmse_score")

        elif self.type == "classification":
            self.test_result = pd.DataFrame(
                self.test_result, columns=["model", "f1_score"]).sort_values(by="f1_score", ascending=False)
